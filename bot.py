...
class Nexto(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        if self.maybe_do_kickoff(packet, ticks_elapsed):
        ...
        if packet.game_info.is_kickoff_pause:
            ...
            try:
                ...
                if self.kickoff_seq is None:
                    print("Initializing Speedflip")
                    self.kickoff_seq = Speedflip(player)

                if player == closest and self.kickoff_seq.is_valid(player, self.game_state):
                    print("Executing Speedflip")
                    self.action = np.asarray(self.kickoff_seq.get_action(player, self.game_state, self.action))
                    self.update_controls(self.action)
                    return True
                else:
                    print("Not closest or Speedflip not valid")
            except Exception as e:
                print(f'Aalltrac - Kickoff sequence failed, falling back to model: {e}')
        else:
            if self.kickoff_seq is not None:
                print("Resetting kickoff sequence")
            self.kickoff_seq = None
        return False
